---
title: "PRF-treatment_effect-0905"
output:
  html_document:
    df_print: paged
...
In this notebook, we will detail each step for statistical inference for treatment effects.
...
The setting is described detailed in the main article 
```{r}
n = 1000
p = 10
...
Split the training data into half: set A and set B
```{r}
## Split data in half
...
On the dataset B, we fit models $\mu_1, \mu_0, \pi$
```{r}
...
On the dataset A, we predict using models and do AIPW transformation.
```{r}
...
# }
# pred_train_1 <- predict(rf_model_1, as.matrix(Xb_sub_1))
# pred_train_0 <- predict(rf_model_0, as.matrix(Xb_sub_0))
