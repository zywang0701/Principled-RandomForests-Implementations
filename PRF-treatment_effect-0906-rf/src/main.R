...
for(isim in 1:nsim){
  ...
  cv_control <- trainControl(method = "cv", number = 10)  # 5-fold cross-validation
  
  # Define the tuning grid for ranger
  tune_grid <- expand.grid(mtry = 3:10, splitrule = "variance", min.node.size = c(5,10))
  
  M = 1000
  # Train Random Forest for Wb == 1 with cross-validation using ranger
  rf_model_1 <- train(
    x = Xb_sub_1, y = Yb_sub_1,
    method = "ranger",
    trControl = cv_control,
    tuneGrid = tune_grid,
    num.trees = M  # Number of trees
  )
  Besttune_1_idx = as.numeric(rownames(rf_model_1$bestTune))
  rmse_1 = rf_model_1$results$RMSE[Besttune_1_idx]
  
  # Train Random Forest for Wb == 0 with cross-validation using ranger
  rf_model_0 <- train(
    x = Xb_sub_0, y = Yb_sub_0,
    method = "ranger",
    trControl = cv_control,
    tuneGrid = tune_grid,
    num.trees = M  # Number of trees
  )
  Besttune_0_idx = as.numeric(rownames(rf_model_0$bestTune))
  rmse_0 = rf_model_0$results$RMSE[Besttune_0_idx]
  
  # Predict on Xa using both models
  piXa <- predict(logistic_model, newdata = as.data.frame(Xa), type = "response")
  mu1Xa <- predict(rf_model_1, as.matrix(Xa))
  mu0Xa <- predict(rf_model_0, as.matrix(Xa))
